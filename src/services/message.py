...
class MessageService:
    ...
    async def process_websocket(self, websocket: WebSocket, client_id: int) -> None:
        ...
        connect_message = CurrentMessageSchema(message=f"{user_name} присоединился(-лась) к чату").model_dump_json()
        disconnect_message = CurrentMessageSchema(message=f"{user_name} вышел(-ла) из чата").model_dump_json()
